...
export default {
  ...
  methods: {
    ...
    async sendMultiSMS() {
        ...
        for (let index = 0; index < this.patients_.length; index++) {
          if (this.patients_[index]?.phone_number) {
            ...
            try {
              ...
              await this.filtersStore.sendMess(
                headers,
                this.patients_[index]?.medical_code
                  ? this.patients_[index].medical_code
                  : "",
      ...
      this.dialogChooseSendMultiple = false;
      // Đóng circle load
      ...
      if (response) {
        ...
        if (response.data?.his_ace_patients?.length > 0) {
          ...
          for (let index = 0; index < tempPatients.length; index++) {
            ...
            if (tempPatients[index]?.medical_code) {
              tempPatient.medical_code = tempPatients[index].medical_code;
            }
    ...
    exportToExcel(pack_info) {
      ...
      for (let index = 0; index < this.patients_.length; index++) {
        const obj = this.patients_[index];
        const tempData = {
          // // Chuẩn cho PA
          // Phone: null,
          // Fullname: null,
          ...
          // Đồng nhất excel import-export
          ["Họ tên"]: null,
          ["Số điện thoại"]: null,
          ["Ngày sinh(dd/mm/yyyy)"]: null,
          Email: null,
          ["Ngày hẹn(dd/mm/yyyy)"]: null,
          ["Khung giờ"]: null,
          ["Link đặt hẹn"]: null,
          // ["Gender (0: Female, 1:Male)"]: 1,
        };
        if (obj.fullname) {
          tempData["Họ tên"] = obj.fullname;
        }
        if (obj.phone_number) {
          tempData["Số điện thoại"] = obj.phone_number;
        }
        if (obj.birthday) {
          tempData["Ngày sinh(dd/mm/yyyy)"] =
            this.filtersStore.DateToExcelSerial(obj.birthday);
        }
        if (obj.email) {
          tempData.Email = obj.email;
        }
        if (obj.date) {
          tempData["Ngày hẹn(dd/mm/yyyy)"] =
            this.filtersStore.DateToExcelSerial(
              this.filtersStore.DataDateToValDate(obj.date)
            );
        }
        if (obj.session_name) {
          tempData["Khung giờ"] =
            this.store.data.shift[Number(obj.session_name) - 1]?.time;
        }
        if (obj.short_url) {
          tempData["Link đặt hẹn"] = obj.short_url;
        }
        data.push(tempData);
      }
      ...
      // Lấy danh sách tiêu đề từ JSON
      const headers = Object.keys(data[0]);

      // Tìm index của cột "Ngày"
      const dateColumnIndex = headers.indexOf("Ngày sinh(dd/mm/yyyy)");
      const dateColumnIndex2 = headers.indexOf("Ngày hẹn(dd/mm/yyyy)");

      if (dateColumnIndex !== -1 && dateColumnIndex2 !== -1) {
        // Xác định phạm vi dữ liệu trong sheet
      ...
      // Danh sách khung giờ
      let timeSlots = [];
      this.store.data.shift.forEach((element) => {
        timeSlots.push(element.time);
      });
      console.log("timeSlots", timeSlots);

      const khungGioColumnIndex = headers.indexOf("Khung giờ"); // Xác định cột "Khung giờ"
      console.log("khungGioColumnIndex", khungGioColumnIndex);
      console.log("this.patients_.length", this.patients_.length);

      if (khungGioColumnIndex !== -1) {
        const columnLetter = XLSX.utils.encode_col(khungGioColumnIndex);
        const sqref = `${columnLetter}2:${columnLetter}${
          this.patients_.length + 1
        }`;
        console.log("sqref", sqref);

        // Thêm dropdown vào worksheet

        ws["!dataValidation"] = ws["!dataValidation"] || [];
        ws["!dataValidation"].push({
          type: "list",
          allowBlank: true, // Cho phép để trống
          formula1: '"07:30 - 09:00,09:00 - 10:30,13:00 - 14:00,14:00 - 15:30"', // Danh sách lựa chọn
          sqref: sqref,
        });
    ...
    async save(notic) {
      ...
      if (!this.editedItem.fullname) {
        ...
        if (this.editedIndex > -1) {
          ...
          if (this.editedItem.medical_code) {
            Patient.medical_code = this.editedItem.medical_code;
          }
                  ...
                  if (Patient.email) {
                    try {
                      ...
                      if (Patient.phone_number && notic && notic === "SMS") {
                        ...
                        await this.filtersStore.sendMessUpdate(
                          headers,
                          Patient.medical_code ? Patient.medical_code : "",
                          Patient.phone_number ? Patient.phone_number : "",
                          Patient.fullname ? Patient.fullname : "",
