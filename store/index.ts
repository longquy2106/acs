...
export const useFiltersStore = defineStore('filterStore', () => {
  ...
  // Hàm chuyển đổi ngày thành số Excel
  function DateToExcelSerial(dateStr: string): number {
    const parts = dateStr.split("-");
    if (parts.length !== 3) throw new Error(`Ngày không hợp lệ: ${dateStr}`);

    // Dùng UTC để tránh bị lệch ngày
    const date = Date.UTC(Number(parts[2]), Number(parts[1]) - 1, Number(parts[0]));
    const excelEpoch = Date.UTC(1899, 11, 30); // Ngày gốc của Excel
    ...
    await axios
      ...
      .then(async (response) => {
        if (response.data?.success) {
          ...
          state.value.snackbar.text = `Tạo link đăng ký cho khách hàng${fullname
            ? " " + fullname
            : ""
            } thành công!`;
          ...
          state.value.snackbar.text = `Tạo link cho khách hàng${fullname
            ? " " + fullname
            : ""
            } không thành công!`;
          state.value.snackbar.state = true;
  ...
  return {
    excelSerialToDate,
    sendEmail, sendEmail2, sendMess, sendMessUpdate,
