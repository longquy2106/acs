<template>
  ...
  >
    ...
    <v-row>
      ...
      >
        <a-button type="primary" @click="browsePacks">Duyệt lịch khám</a-button>
        ...
        reader.onload = async (e) => {
          ...
          if (sheetData) {
            ...
            for (let index = 0; index < sheetData.length; index++) {
              ...
              if (this.cmp_patients_.length > 0) {
                console.log(
                  "this.cmp_patients_.length",
                  this.cmp_patients_.length
                );
                let hasDuplicate = false;
                for (
                  let index = 0;
                  index < this.cmp_patients_.length;
                  index++
                ) {
                  const oldItem = this.cmp_patients_[index];

                  // console.log("this.cmp_patients_", this.cmp_patients_);

                  // console.log(
                  //   "tempPatient.fullname",
                  //   oldItem.email === tempPatient.email
                  // );
                  // console.log("oldItem.email", oldItem.email);
                  // console.log("tempPatient.email", tempPatient.email);
                  if (
                    ((!oldItem.fullname && !tempPatient.fullname) ||
                      oldItem.fullname === tempPatient.fullname) &&
                    ((!oldItem.phone_number && !tempPatient.phone_number) ||
                      oldItem.phone_number === tempPatient.phone_number) &&
                    ((!oldItem.birthday && !tempPatient.birthday) ||
                      oldItem.birthday ===
            ...
            if (this.patient_[0]?.fullname) {
              this.dialogImport = true;
            } else {
              console.log("Không có data khách hàng mới!");
              this.snackbarDefault("Không có data khách hàng mới!");
            }
          }
        };
      ...
      if (response) {
        ...
        if (response.data?.his_ace_patients?.length > 0) {
          ...
          for (let index = 0; index < tempPatients.length; index++) {
            ...
            this.cmp_patients_.push(tempPatient);
          }
          console.log("this.cmp_patients_", this.cmp_patients_);
        }
      }
